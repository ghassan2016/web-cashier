...
type Count = { id: number; number: string; warehouse_id: number; count_date: string | null; status: string };
type Ref = { id: number; name: string };
type Line = { item_id: string; counted_qty: string };
...
export default function StockCountsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setList((await api<{ data: Count[] }>("/stock-counts?per_page=50")).data ?? []); }
  ...
  async function save() {
    ...
    try {
      await api("/stock-counts", { method: "POST", body: {
        warehouse_id: Number(form.warehouse_id),
        lines: form.lines.map((l) => ({ item_id: Number(l.item_id), counted_qty: Number(l.counted_qty) })).filter((l) => l.item_id),
      } });
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-lg font-bold text-slate-800">الجرد الفعلي</h1>
        <button onClick={() => setForm({ warehouse_id: "", lines: [{ item_id: "", counted_qty: "" }] })} className="rounded-lg bg-[#0E7C66] px-4 py-2 text-sm text-white hover:bg-[#0A5C4C]">+ جرد</button>
      </div>
      <div className="overflow-hidden rounded-xl border bg-white">
        <table className="w-full text-right text-sm">
          <thead className="bg-slate-100 text-slate-600"><tr>
            <th className="px-4 py-3 font-medium">الرقم</th><th className="px-4 py-3 font-medium">التاريخ</th><th className="px-4 py-3 font-medium">الحالة</th>
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">جارٍ التحميل…</td></tr>
            : list.length === 0 ? <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400">لا توجد عمليات جرد</td></tr>
            : list.map((c) => (
              <tr key={c.id} className="border-t">
                <td className="px-4 py-3 font-medium text-[#0E7C66]">{c.number}</td>
                <td className="px-4 py-3 text-slate-600">{c.count_date ?? "—"}</td>
      ...
      {form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setForm(null)}>
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="mb-1 font-bold text-slate-800">جرد فعلي</h2>
            <p className="mb-3 text-xs text-slate-500">يُسوّي المخزون إلى الكمية المعدودة.</p>
            ...
            <div className="space-y-2 rounded-lg border p-3">
              {form.lines.map((l, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <input type="number" step="0.01" placeholder="المعدود" value={l.counted_qty} onChange={(e) => setForm({ ...form, lines: form.lines.map((x, j) => j === i ? { ...x, counted_qty: e.target.value } : x) })} className="w-24 rounded-lg border px-2 py-1 text-sm" />
                  <button onClick={() => setForm({ ...form, lines: form.lines.filter((_, j) => j !== i) })} className="text-red-600">✕</button>
                </div>
              ))}
              <button onClick={() => setForm({ ...form, lines: [...form.lines, { item_id: "", counted_qty: "" }] })} className="text-sm text-[#0E7C66] hover:underline">+ صنف</button>
            </div>
